...
const binding = () => ({
  plan_hash: digest('1'), git_head: 'a'.repeat(40), package_version: '1.3.2',
...
test('current binding derives repository identity and rejects tracked dirty input', (t) => {
  ...
  fs.writeFileSync(path.join(root, 'package.json'), '{"version":"1.3.2"}\n');
  ...
  assert.equal(current.package_version, '1.3.2');
