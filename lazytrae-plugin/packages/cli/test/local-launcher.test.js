...
test('permanent local launcher reports v1.2.2 and owns both package bins', () => {
...
test('local release launcher initializes a project from an unrelated cwd when paths contain spaces', () => {
  ...
  try {
    ...
    assert.equal(JSON.parse(mcp.stdout.trim()).result.serverInfo.version, '1.2.2');
  } finally {
...
test('release-owned launcher serves MCP initialize from outside the release with a node-only PATH', () => {
  ...
  try {
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.equal(JSON.parse(result.stdout.trim()).result.serverInfo.version, '1.2.2');
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
