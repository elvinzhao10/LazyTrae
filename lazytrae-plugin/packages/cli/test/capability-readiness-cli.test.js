...
const schema = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'contracts', 'lazyseries-capability-readiness.v1.json'), 'utf8'));
const expectedText = [
...
function validate(record) {
  ...
  assert.equal(record.contract_digest, schema.properties.contract_digest.const);
  assert.equal(record.host, 'lazytrae');
...
test('capability-status --json returns schema-valid records without writing report inputs', () => {
  withFixture('lazytrae-readiness-json-', fixture => {
    ...
    assert.equal(records.find(record => record.capability === 'architecture_search').status, 'disabled');
    assert.equal(records.find(record => record.capability === 'documentation_search').status, 'disabled');
    assert.equal(records.find(record => record.capability === 'browser_automation').status, 'disabled');
...
test('capability-status --json reports an absent tooling state as not-initialized without creating it', () => {
  withFixture('lazytrae-readiness-uninitialized-', fixture => {
    ...
    const records = JSON.parse(result.stdout);
    assert.equal(records.length, 9);
    assert.ok(records.every(record => record.status === 'not-initialized'));
...
test('capability-status --json requires an executable receipt-owned CodeGraph provider', () => {
  withFixture('lazytrae-readiness-owned-', fixture => {
    ...
    assert.equal(codegraph.status, 'incompatible');
    ...
    assert.equal(executableCodegraph.status, 'owned-ready');
    assert.deepEqual(executableCodegraph.receipt, { owner: 'lazytrae-tooling', schema_version: 1, state: 'ready' });
...
test('capability-status --json fails safely when the readiness contract integrity check fails', () => {
  withFixture('lazytrae-readiness-contract-', fixture => {
    ...
    fs.copyFileSync(path.join(__dirname, '..', 'contracts', 'lazyseries-capability-readiness.v1.json'), contractPath);
    ...
    assert.ok(report.every(record => record.status === 'failed-optional'));
...
test('capability-status --json treats malformed state as a report-only failed optional result', () => {
  withFixture('lazytrae-readiness-malformed-', fixture => {
    ...
    assert.ok(records(result).every(record => record.status === 'failed-optional'));
