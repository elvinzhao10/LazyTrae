...
test('loop CLI creates a review blocker after three normalized same-criterion failures', () => {
  ...
  for (const reason of ['Same Criterion Failure', ' same criterion   failure ', 'same criterion failure']) {
    const result = runCli(['loop', 'record-evidence', 'goal-1', 'crit-1', '.lazytrae/evidence/proof.txt', '--status', 'fail', '--reason', reason], { cwd: fixture });
...
test('loop checkpoint retains the active goal as checkpoint provenance', () => {
  ...
  assert.equal(runCli(['loop', 'create-goals', '--brief', '.lazytrae/evidence/brief.md', '--goal-id', 'goal-1', '--criterion-id', 'goal-1-criterion'], { cwd: fixture }).status, 0);
  ...
  assert.equal(runCli(['loop', 'complete-goals'], { cwd: fixture }).status, 0);
  assert.equal(runCli(['loop', 'record-evidence', 'goal-1', 'goal-1-criterion', '.lazytrae/evidence/proof.txt'], { cwd: fixture }).status, 0);
...
test('loop steer supports audit-only annotate_ledger with evidence and rationale', () => {
  ...
  const result = runCli(['loop', 'steer', '--kind', 'annotate_ledger', '--evidence', '.lazytrae/evidence/proof.txt', '--rationale', 'audit note'], { cwd: fixture });
  assert.equal(result.status, 0);
  const events = fs.readFileSync(path.join(fixture, '.lazytrae', 'logs', 'loop-events.ndjson'), 'utf-8');
