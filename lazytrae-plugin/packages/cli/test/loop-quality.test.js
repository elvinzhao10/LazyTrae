...
test('validateQualityGate rejects missing manualQa.surfaceEvidence', () => {
  ...
  assertQualityGateError(fixture, '.lazytrae/evidence/bad-quality.json', /manualQa\.surfaceEvidence/);
});
...
test('validateQualityGate rejects gateReview.blockers', () => {
  ...
  writeCanonicalQualityGate(fixture, '.lazytrae/evidence/gate-blockers.json', gate);

  assertQualityGateError(fixture, '.lazytrae/evidence/gate-blockers.json', /gateReview\.blockers/);
});

test('validateQualityGate rejects a missing artifact path', () => {
  const fixture = makeLoopFixture('lazytrae-quality-missing-artifact-');
  const gate = makeCanonicalQualityGate();
  gate.manualQa.artifactRefs[0].path = '.lazytrae/evidence/missing-artifact.txt';
  writeJson(fixture, '.lazytrae/evidence/missing-artifact-gate.json', gate);

  assertQualityGateError(fixture, '.lazytrae/evidence/missing-artifact-gate.json', /manualQa\.artifactRefs\[0\]\.path/);
});
...
test('validateQualityGate rejects criteriaCoverage passCount below totalCriteria', () => {
  ...
  writeCanonicalQualityGate(fixture, '.lazytrae/evidence/bad-coverage.json', gate);

  assertQualityGateError(fixture, '.lazytrae/evidence/bad-coverage.json', /criteriaCoverage\.passCount/);
});
