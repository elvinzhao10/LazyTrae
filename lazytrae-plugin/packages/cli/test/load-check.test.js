...
test('load-check reports v1.2.3 package readiness separately from unverified IDE registration', () => {
  withFixture('lazytrae-load-check-ready-', fixture => {
    const result = runCli(['load-check', '--host', 'ide'], { cwd: fixture });

    assert.equal(result.status, 0, result.stdout);
    assert.match(result.stdout, /LazyTrae Tool Load Check — v1\.2\.3 Package Readiness/);
    assert.match(result.stdout, /PASS Machine status v2: version=1\.2\.3/);
...
test('load-check help identifies the v1.2.3 package readiness check', () => {
  const result = runCli(['load-check', '--help']);

  assert.equal(result.status, 0, result.stdout);
  assert.match(result.stdout, /Check v1\.2\.3 package readiness after initialization\./);
  assert.doesNotMatch(result.stdout, /v0\.17/);
});
