...
function install(root) {
  ...
  if (result.error) throw new Error(`tooling npm install failed: ${result.error.message}`);
  if (result.status !== 0) throw new Error(`tooling npm install failed: ${result.stderr || result.stdout}`.trim());
