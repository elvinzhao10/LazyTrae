...
function approvalClasses(text, context) {
  ...
  const rules = [
    ...
    ['remote-data-egress', /\b(upload|send|egress)\b.*\b(repo|repository|source|data)\b|\bremote data\b/i, /\bdo not\s+(upload|send)/i],
    ...
    ['credentials-auth-or-paid-service', /\b(use|enter|change)\b.*\b(credentials?|password|paid service|api key)\b/i, /\bdo not\s+(use|enter|change)/i],
