...
function checkTraeStructure(repoRoot) {
  ...
  if (fs.existsSync(hooksPath)) {
    ...
    results.push({ label: '.trae/hooks.json', status: 'WARN', detail: 'Hooks config for v1.1.0' });
  }
  ...
  if (fs.existsSync(mcpPath)) {
    ...
    results.push({ label: '.trae/mcp.json', status: 'WARN', detail: 'MCP config for v1.1.0' });
  }
