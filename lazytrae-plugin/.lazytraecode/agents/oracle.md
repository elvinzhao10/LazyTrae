...
- The plan file that was executed (from `.omo/plans/` or `.lazytraework/plans/`)
...
- `.lazytraework/evidence/` — any existing verification evidence
...
- **PostCompact hook**: Trae has no PostCompact hook event. Evidence from `.lazytraework/evidence/` files must be re-read after compaction.
...
- Write the deliverable report to `.lazytraework/team/members/<id>/report.md`
- Use `WORKING:` / `BLOCKED:` heartbeat markers in `.lazytraework/team/mailbox/<id>/outbox.md`
