...
- `.lazytraework/state/active-loop.json` — current loop state (if continuing)
- `.lazytraework/state/boulder.json` — current boulder state (if executing)
...
| `create_goal` | `# Goal` block in response | Write goal block or update `.lazytraework/state/active-loop.json` |
...
- **PostCompact hook**: Trae has no PostCompact hook event. State recovery relies on durable `.lazytraework/state/` files. Re-read state files after any compaction.
