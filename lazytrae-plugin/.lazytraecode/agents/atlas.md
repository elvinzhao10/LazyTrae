...
- The plan file being executed (from `.omo/plans/` or `.lazytraework/plans/`)
- `.lazytraework/state/boulder.json` — current boulder state
...
- **PostCompact hook**: Trae has no PostCompact hook event. State recovery relies on durable `.lazytraework/state/` files.
...
- Record evidence in `.lazytraework/evidence/` or in the commit message
