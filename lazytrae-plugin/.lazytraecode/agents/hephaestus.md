...
- `.lazytraework/state/boulder.json` — if executing under a plan
...
- **PostCompact hook**: Trae has no PostCompact hook event. State recovery relies on durable notepad and `.lazytraework/state/` files. Always maintain a notepad for context recovery.
