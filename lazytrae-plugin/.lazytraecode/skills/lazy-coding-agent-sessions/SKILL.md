...
Reconstruct past sessions so agents can answer "what did we already do" questions without relying on memory. The LazyTrae sessions store (`.lazytraework/state/sessions.json`) is the primary source; platform transcript files supplement when the user asks about non-Trae sessions.

## Required Context to Inspect

- `.lazytraework/state/sessions.json` — LazyTrae session tracking (primary store).
- `.lazytraework/state/boulder.json` — active work-in-progress state.
- `.lazytraework/state/active-loop.json` — active ulw-loop plan with goal statuses.
- `.lazytraework/evidence/` — recorded evidence from past verification runs.
...
Read `.lazytraework/state/sessions.json` to inspect tracked Trae sessions:

```
Read .lazytraework/state/sessions.json
```
...
```
Grep "keyword" .lazytraework/state/sessions.json
Grep "keyword" .lazytraework/evidence/
```
...
- Read `.lazytraework/state/boulder.json` for task-by-task progress.
- Read `.lazytraework/state/active-loop.json` for goal and criterion statuses.
- Read `.lazytraework/evidence/` files named after the work for verification results.
...
- Read `.lazytraework/state/sessions.json`, `.lazytraework/state/boulder.json`, `.lazytraework/state/active-loop.json`.
- Search `.lazytraework/evidence/` for past verification artifacts.
...
- If `.lazytraework/state/sessions.json` is empty or absent: report that no Trae sessions have been tracked yet.
...
Platform: Trae
Store: .lazytraework/state/sessions.json
Sessions Found: {N}
