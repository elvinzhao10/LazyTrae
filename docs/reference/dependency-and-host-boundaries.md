...
This page answers a common source-reading question: which behavior is supplied by LazyTrae itself, which behavior is borrowed from an installed dependency, and which behavior belongs entirely to Trae IDE, Work, or CLI?

```mermaid
flowchart TB
    Host["Trae IDE / Work / CLI"]
...
| Host runtime | Project discovery, Work skill loading, CLI MCP registration, hook delivery | Trae IDE, Work, or CLI | Only inside the selected host/session | Nothing until the user observes it. |
